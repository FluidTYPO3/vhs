...
class Tx_Vhs_ViewHelpers_Media_SizeViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper {
	...
	public function render() {
		...
		if (!file_exists($file) || is_dir($file)) {
			throw new \TYPO3\CMS\Fluid\Core\ViewHelper\Exception('Cannot determine size of "' . $file . '". File does not exist or is a directory.', 1356953963);
		}

		$size = filesize($file);

		if ($size === FALSE) {
			throw new \TYPO3\CMS\Fluid\Core\ViewHelper\Exception('Cannot determine size of "' . $file . '".', 1356954032);
		}
