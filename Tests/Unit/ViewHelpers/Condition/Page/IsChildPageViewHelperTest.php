...
class IsChildPageViewHelperTest extends AbstractViewHelperTest {
	...
	public function testRender() {
		$GLOBALS['TYPO3_DB'] = $this->getMock('TYPO3\\CMS\\Core\\Database\\DatabaseConnection', ['exec_SELECTquery'], [], '', FALSE);
		$GLOBALS['TYPO3_DB']->expects($this->any())->method('exec_SELECTquery')->will($this->returnValue(FALSE));
		$this->assertEquals('else', $this->executeViewHelper(['then' => 'then', 'else' => 'else', 'pageUid' => 0]));
	}
